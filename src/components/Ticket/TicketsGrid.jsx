...
const TicketsGrid = ({ inProgress, completeTask, ticketsPromise, handleInProgress }) => {
    ...
    return (
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4 shadow-xl  shadow-gray-500'>
            {visibleTickets.map((customerTicket) => {
                return (
                    <TicketCard key={customerTicket?.id} customerTicket={customerTicket} inProgress={inProgress} handleInProgress={handleInProgress} />
                )
            })}
        </div>
    );
