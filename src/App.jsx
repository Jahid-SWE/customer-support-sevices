...
import InProgress from './components/Ticket/InProgress/InProgress'
import Resolved from './components/Ticket/Resolved/Resolved'
import TicketsGrid from './components/Ticket/TicketsGrid'
import InProgressTasks from './components/Ticket/InProgress/InProgressTasks'
import ResolvedTask from './components/Ticket/Resolved/ResolvedTask'
...
function App() {
  ...
  const handleInProgress = (customerTicket) => {
    const isProgressAdded = inProgress.some((tkt) => tkt?.id === customerTicket?.id);

    if (isProgressAdded) {
      return toast.error("Already ticket In-Progress");
    }

    setInProgress([...inProgress, customerTicket]);
    toast.success("Added In-Progress!");
  }
